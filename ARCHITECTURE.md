...
- `src/integrations/supabase/{client.ts,client.server.ts,auth-middleware.ts}`
  ...
  safe to consolidate).
- `bun run dev` still doesn't boot in this sandbox (see `AUDIT.md`), so
  nothing UI-level has been visually verified here — `bun run build` does
  work now, though (the Windows path-separator bug tracked here was in
  `@lovable.dev/mcp-js`'s Vite plugin, removed along with the MCP feature).
