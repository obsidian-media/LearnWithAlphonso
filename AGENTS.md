...
| `ios/LearnWithAlphonso/Sources/DesignSystem/` | Meadow-theme design system (colors/spacing/radius tokens ported from `src/styles.css`'s oklch values, Fraunces/Geist as bundled variable fonts resolved via CoreText variation axes, button styles incl. the `.hard-shadow` press effect, `SpringEntrance`). Applied across every screen — see `ios/LearnWithAlphonso/Sources/Fonts/` for the bundled `.ttf` files and `Info.plist`'s `UIAppFonts` for registration. Before this, the app had zero design-system files and looked like stock SwiftUI throughout |
