...
| `ios/LearnWithAlphonso/Sources/ListenView.swift`                                                 | Podcast browser: owns the NavigationStack; the folder listing it pushes deliberately does not, so stacks never nest. Offline it shows the downloaded set flat and title-ordered, with a distinct state for offline-with-nothing-downloaded                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `ios/LearnWithAlphonso/Sources/PodcastDownloadManager.swift`                                     | App-target wiring for offline download: Application Support directory, backup exclusion, staging -> verify byte count -> atomic move -> write row, and `reconcile()` at launch. Deliberately decides nothing -- every rule is delegated to `PodcastCache`/`PodcastCacheBudget`, because no test in this repo can reach the app target                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `ios/LearnWithAlphonsoKit/Sources/LearnWithAlphonsoKit/PodcastCache.swift`                       | The offline-download rules, in the Kit so `ios-swift-tests` can reach them: filenames (server ids are sanitised -- they are not trusted as path components), staleness by ETag (Supabase's IS the content MD5), duration reconciliation, the flat offline listing, and the budget. The budget is a parameter everywhere, never read from the default, so the refusal path is reachable in a test. Nothing here deletes anything                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `src/lib/admin-auth.ts`                                                                          | `isAdminUser` -- the one authorization decision in the admin subsystem. MUST be called with a service-role client: `admin_users` has RLS on and no policies, so a user's own token reads nothing from it. Fails closed on absent row, query error and empty subject; filters on `user_id` alone, never a claim a user can change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `src/lib/admin-middleware.ts`                                                                    | `requireAdmin` -- chains `requireSupabaseAuth`, then the allowlist. Throws a message indistinguishable from an ordinary auth failure, on purpose: a distinct refusal confirms both that the endpoint exists and that the token was valid                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `src/lib/admin.functions.ts`                                                                     | Every admin server function, in ONE file so `admin.functions.test.ts` has one thing to enumerate. Adding an admin function anywhere else defeats that test. Keep `ADMIN_FUNCTION_NAMES` in sync -- the test checks both directions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `src/lib/admin.functions.ts` (`affectedOrThrow`, `cycleFor`)                                     | `affectedOrThrow` refuses a write that matched no row -- PostgREST succeeds with zero rows touched, so every mutation used to report success for a vanished id. A NULL count counts as nothing-matched, so a caller that stops asking for the count fails loudly. `cycleFor` returns the path `findCycle` produces, so the move refusal can name the loop                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `src/lib/admin-upload.ts`                                                                        | Content-based audio sniffing and the size cap. The extension is never consulted: the bucket is public-read and served from our own domain. Allowlist of known-good signatures, not a denylist                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `vite.admin.config.ts`                                                                           | The admin build. `srcDirectory: "admin"` is what moves the app; `routesDirectory` alone does nothing since it resolves relative to it. `admin/routeTree.gen.ts` is committed because the generator only runs under `vite dev`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
bun run test         # Vitest. 146 files / 1,288 tests as of 2026-09-26,
...
bun run dev:admin    # the admin app, separate Vite config (admin/ routes)
bun run build:admin  # ditto -- what the admin Vercel project builds
...
### Compliance guards — new 2026-09-26, and why each exists

Four App Store requirements now have tests rather than intentions. Each
was written because the absence of it was a live defect, not
speculatively.

| Guard                                             | Holds                                                                                                | The failure it prevents                                                                                                                                                                    |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `legal-pages.test.tsx`                            | Hector disclosed, Deepgram and NVIDIA named, export/delete both described, contact addresses correct | The policy silently drifting from the code. **This file once asserted the WRONG email** and so protected stale branding — a guard enforcing the bug.                                       |
| `apple-revocation.test.ts`                        | ES256 client secret is 64-byte `r                                                                    |                                                                                                                                                                                            | s` and verifies | Node signs **DER** by default; Apple answers `invalid_client` and never says the encoding is why. Mutation-tested: removing `dsaEncoding` turns exactly this test red. |
| `account.functions.test.ts` — `NOT_PERSONAL_DATA` | Every `user_id` table is exported or excluded **with a written reason**                              | A table quietly escaping GDPR export. The reason requirement is the point: `apple_auth_tokens` is excluded because exporting a live credential is a security hole dressed as transparency. |
| `account.functions.test.ts` — return shape        | `deleteMyAccount` returns `{ deleted, appleRevoked }`                                                | `appleRevoked` silently disappearing, which would make an unrevoked Apple grant invisible.                                                                                                 |

**Two things these encode that are easy to lose:**

- **A guard that stores without acting is not a guard.** `blocked_users`
  is enforced at seven call sites, not just written to. A block that
  records a row and still shows the blocked user on a leaderboard is
  the same defect as a test that cannot fail.
- **An exclusion list needs a reason per entry, in writing.** It is a
  hole in a guard by construction; the reason is what stops it becoming
  a dumping ground.
