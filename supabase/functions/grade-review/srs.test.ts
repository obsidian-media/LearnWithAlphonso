...
Deno.test("computeReviewGrade halves (rather than zeros) repetitions on a wrong answer, and records a lapse", () => {
  const result = computeReviewGrade({ correct: false, ease: 2.3, intervalDays: 6, repetitions: 2, lapses: 1, elapsedDays: 6 });
...
Deno.test("computeReviewGrade drops all the way to a fresh restart when repetitions halves to zero", () => {
  const result = computeReviewGrade({ correct: false, ease: 2.3, intervalDays: 1, repetitions: 1, lapses: 0, elapsedDays: 1 });
...
Deno.test("computeReviewGrade sets a 1-day interval on the first correct repetition", () => {
  const result = computeReviewGrade({ correct: true, ease: 2.3, intervalDays: 0, repetitions: 0, lapses: 0, elapsedDays: 0 });
  assertEquals(result.retired, false);
...
Deno.test("computeReviewGrade sets a 3-day interval on the second correct repetition", () => {
  const result = computeReviewGrade({ correct: true, ease: 2.45, intervalDays: 1, repetitions: 1, lapses: 0, elapsedDays: 1 });
  assertEquals(result.intervalDays, 3);
  assertEquals(result.repetitions, 2);
});

Deno.test("computeReviewGrade grows the interval by ease on the third correct repetition when reviewed on schedule", () => {
  const result = computeReviewGrade({ correct: true, ease: 2.6, intervalDays: 3, repetitions: 2, lapses: 0, elapsedDays: 3 });
  assertEquals(result.repetitions, 3);
  assertEquals(result.intervalDays, Math.round(3 * 2.75));
});

Deno.test("computeReviewGrade grows the interval further when the review happens well past its due date", () => {
  const onTime = computeReviewGrade({ correct: true, ease: 2.6, intervalDays: 3, repetitions: 2, lapses: 0, elapsedDays: 3 });
  const wayOverdue = computeReviewGrade({ correct: true, ease: 2.6, intervalDays: 3, repetitions: 2, lapses: 0, elapsedDays: 30 });
  assertEquals(wayOverdue.intervalDays, Math.round(3 * 2.75 * 1.5));
  assert(wayOverdue.intervalDays > onTime.intervalDays);
});

Deno.test("computeReviewGrade retires the item after 4 clean repetitions in a row", () => {
  const result = computeReviewGrade({ correct: true, ease: 2.75, intervalDays: 8, repetitions: 3, lapses: 0, elapsedDays: 8 });
...
Deno.test("computeReviewGrade caps ease at 2.8 so it never grows unbounded", () => {
  const result = computeReviewGrade({ correct: true, ease: 2.75, intervalDays: 10, repetitions: 1, lapses: 0, elapsedDays: 10 });
...
Deno.test("computeReviewGrade falls back to a 6-day interval if the growth formula rounds to zero", () => {
  const result = computeReviewGrade({ correct: true, ease: 1.3, intervalDays: 0, repetitions: 2, lapses: 0, elapsedDays: 0 });
