...
/// A minimal, dependency-free SHA-256 (FIPS 180-4), used only to derive
/// GoogleOAuthFlow's PKCE `code_challenge` from a verifier. Hand-written
/// rather than reaching for CryptoKit so this package keeps building and
/// testing on any platform with a Swift toolchain, including Windows --
/// CryptoKit is Apple-platform-only. See Package.swift's doc comment.
enum SHA256Pure {
    ...
    static func hash(_ message: Data) -> Data {
        ...
        while chunkStart < padded.count {
            ...
            for i in 0..<16 {
                ...
                w[i] = UInt32(chunk[base]) << 24 | UInt32(chunk[base + 1]) << 16
                    | UInt32(chunk[base + 2]) << 8 | UInt32(chunk[base + 3])
            }
            for i in 16..<64 {
