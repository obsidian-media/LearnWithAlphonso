...
The iOS port needed a fix mid-implementation last time for `text-*-on-ember` contrast (bright coral fill + light text failing WCAG). The same defect exists on web, and it's **wider** here: `grep -rn "bg-ember" src --include="*.tsx"` finds ~15 files pairing `bg-ember` with hardcoded `text-surface` (white-on-coral). For the three existing themes this renders fine (their `ember` is dark/medium — Meadow's own burnt orange, or literally `= moss` for Studio Ink/Manuscript). For Canopy's bright coral, `text-surface` (`#EFFAF4`) on `bg-ember` (`#E4573F`) computes to **3.43:1** (corrected here after code review — an earlier draft of this section said "~2.7:1", computed against a wrong assumed `surface`; the real number is still a real failure, just not quite as bad as first stated), the same class of failure the iOS spec rejected.

**Post-review addendum**: `text-ink-on-ember`'s Canopy value (`ink` = `#05261A`) on `ember` computes to **4.41:1** — a real, verified improvement over 3.43:1, but it falls just short of the 4.5:1 normal-text WCAG AA threshold for the `text-sm font-semibold` (14px) and smaller sizes this token is actually used at. Ruling, made after a fresh review caught this: not fixing further in this PR. Darkening `ember` enough to clear 4.5:1 would diverge Canopy's coral from the value already shipped and TestFlight-tested on iOS (`AlphonsoTheme.swift`'s `ember: Color(hex: 0xE4_57_3F)`) — a cross-platform color change is a bigger, coordinated decision than this port should make unilaterally. 4.41:1 is the accepted, documented state for now; a future pass that revisits Canopy's palette on both platforms together should treat "get `ember` to a clean 4.5:1" as a real requirement, not re-discover this gap from scratch.
